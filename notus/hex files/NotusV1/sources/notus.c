...
int8_t GetTurbineRPM(sSensorValues_t* sSensorValues)
{
    ...
    {
        sSensorValues->fTurbineRPM = fMeanTurbineRPM/100;
...
{
    int8_t bError = 0;
    sSensorValues->fWheelRPM = (sSensorRawFreq.fWheelFreq*60.0f)/GEARTOOTH_WHEEL;
    return bError;
}
...
void PrintEeprom(void)
{
    ...
    sprintf(bString, "\n\rGear = %u Hz\n\rTurbine Direction = %f degres", ReadGearEeprom(),ReadTurbineDirEeprom());
    UartTxFrame(UART_1,bString,sizeof(bString));
}
