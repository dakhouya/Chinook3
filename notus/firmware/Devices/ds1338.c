...
void ds1338_init(ds1338_s* ds1338, int i2c_port) {
    ...
    I2C_Send(DS1338_WRITE,DS1338_CONTROLS_REG, 0x00, ds1338->port);
}
...
void ds1338_setDate(ds1338_s* ds1338, uint8_t year,
    ...
    I2C_Send(DS1338_WRITE,DS1338_YEARS_REG, y.year, ds1338->port);
    I2C_Send(DS1338_WRITE,DS1338_MONTHS_REG, m.month, ds1338->port);
    I2C_Send(DS1338_WRITE,DS1338_DATES_REG, d.date, ds1338->port);
}
void ds1338_setClock(ds1338_s* ds1338, uint8_t hours,
    ...
    I2C_Send(DS1338_WRITE,DS1338_HOURS_REG ,h.hours24, ds1338->port);
    I2C_Send(DS1338_WRITE,DS1338_MINUTES_REG ,m.minutes, ds1338->port);
    I2C_Send(DS1338_WRITE,DS1338_SECONDS_REG ,s.seconds, ds1338->port);
}
...
void ds1338_GetClock(ds1338_s* ds1338)
{
    ...
    I2C_Receive(DS1338_WRITE,DS1338_READ,DS1338_SECONDS_REG,seconds, ds1338->port);
    I2C_Receive(DS1338_WRITE,DS1338_READ,DS1338_MINUTES_REG,minutes, ds1338->port);
    I2C_Receive(DS1338_WRITE,DS1338_READ,DS1338_HOURS_REG,hours, ds1338->port);

    ds1338->seconds = seconds;//10U*(seconds & 0x70)+(seconds & 0x0F);
    ds1338->minutes = minutes;//10U*(minutes & 0x70)+(minutes & 0x0F);
    ds1338->hours   = hours;//20U*(hours & 0x20) + 10U*(hours & 0x10)+(hours & 0x0F);
}

void ds1338_GetDate(ds1338_s* ds1338)
{
    I2C_Receive(DS1338_WRITE,DS1338_READ,DS1338_DAYS_REG,(uint8_t)ds1338->day, ds1338->port);
    I2C_Receive(DS1338_WRITE,DS1338_READ,DS1338_MONTHS_REG,(uint8_t)ds1338->month, ds1338->port);
    I2C_Receive(DS1338_WRITE,DS1338_READ,DS1338_YEARS_REG,(uint8_t)ds1338->year, ds1338->port);
}
