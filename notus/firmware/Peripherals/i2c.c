...
/************************************************************/
/*
I2C_Send16bitsRegister
	Send data to a slave on I2C bus in a 16 bits register

	INPUT 		: 
				ubAddress   : Slave address
				usRegister  : 16bits Slave Register
				ubData      : Data that will be written in the register
...
*/
/************************************************************/
bool I2C_Send16bitsRegister(uint8_t ubAddress, uint16_t usRegister, uint8_t ubData, uint8_t ubI2cNo)
{
	uint8_t ubValid = FALSE;
	...
	if(ubValid)
	{
		...
		*I2cxtrn[ubI2cNo]=(uint8_t)((usRegister>>8) & 0x00FF);//Slave Register Address
		I2CFlag(ubI2cNo);

		*I2cxtrn[ubI2cNo]=(uint8_t)(usRegister & 0x00FF);//Slave Register Address
		I2CFlag(ubI2cNo);
...
bool I2C_Send16bitsRegisterMultiple(uint8_t ubAddress, uint16_t usRegister, uint8_t* ubData, uint8_t ubDataLenght, uint8_t ubI2cNo)
{
        uint8_t ubValid = FALSE;
        uint8_t i; //counter
	/*Validity check*/
	...
	if(ubValid)
	{
		...
		*I2cxtrn[ubI2cNo]=(uint8_t)(usRegister & 0x00FF);//Slave Register Address
		I2CFlag(ubI2cNo);
                ...
                for(i=0;i<ubDataLenght;i++)
                {
                    *I2cxtrn[ubI2cNo]=ubData[i];//write data to selected register
                    I2CFlag(ubI2cNo);
                }
...
/*
I2C_receive
	Read data from a slave with 16bits register address on I2C bus
...
*/
/************************************************************/
bool I2C_Receive16bitsRegister(uint8_t ubAddressWrite,uint8_t ubAddressRead,uint16_t usRegister,uint8_t* ubData, uint8_t ubI2cNo)
{
	uint8_t ubValid = FALSE;
